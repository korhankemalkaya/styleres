...
class ResLayers(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.activation_layer(res + shortcut)

class E1(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        t_channel = 512
        self.first_layer = nn.Conv2d(in_channels, t_channel, kernel_size=3, padding=1, bias=True)
        self.convs = nn.Sequential(*[ResLayers(t_channel,t_channel,1, 'lrelu'), 
                                    ResLayers(t_channel,out_channels,1, 'lrelu'), 
...
class E2(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = nn.Sequential(*[ResLayers(in_channels,in_channels,1, 'lrelu'), 
                                     ResLayers(in_channels,out_channels,1, 'lrelu'), 
                                     ResLayers(out_channels,out_channels,1, 'lrelu'),
                                     ResLayers(out_channels,out_channels,1, 'lrelu'),
                                     ResLayers(out_channels,out_channels,1, ) ])


    def forward(self, generator_feats, e1_feats):
        deltaF = torch.cat((self.down1(generator_feats), self.down2(e1_feats)), dim=1)
        deltaF = self.convs(deltaF)
        return deltaF
    ...
    def __init__(self,
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if resolution == embed_res:
            ...
            self.modify_e1 = E1(in_channels=in_c, out_channels=512)
            self.modify_e2 = E2(in_channels=512, out_channels=512)
    ...
    def forward(self, x, img, ws, conditions=None, noise=None, weight_deltas = None, highres_outs=None, return_f = False,
        ...
        if self.in_channels == 0:
            ...
            if x.shape[-1] == self.embed_res and highres_outs is not None: 
                aligned_feats = self.modify_e1(highres_outs[f'{self.embed_res}x{self.embed_res}'], highres_outs['inversion'])
                deltaF = self.modify_e2(x, aligned_feats)
                x = x + deltaF
                gouts['gates'] = 0
