...
class FeatureExtraction(nn.Module):
    def __init__(self,in_channels, out_channels ):
        ...
        t_channel = 512
        self.first_layer = nn.Conv2d(in_channels, t_channel, kernel_size=1, padding=0, bias=True)
        self.convs = nn.Sequential(*[ResLayers(t_channel,t_channel,1), ResLayers(t_channel,out_channels,1), 
                                    ResLayers(out_channels,out_channels,1), ResLayers(t_channel,t_channel,1),
...
class GateNetwork(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = nn.Sequential(*[ResLayers(1024,896,1), ResLayers(896,768,1), ResLayers(768,640,1),
                                    ResLayers(640,512,1), ResLayers(512,512,1), ResLayers(512,512,1),
                                    ResLayers(512,512,1, False)  ])
        #self.convs2 = nn.Sequential(*[ResLayers(512,in_channels,1), ResLayers(in_channels,out_channels,1), ResLayers(out_channels,1,1) ])
    ...
    def forward(self, x, img, ws, conditions=None, noise=None, weight_deltas = None, highres_outs=None, return_f = False,
        ...
        if self.in_channels == 0:
            ...
            if x.shape[-1] == self.embed_res and highres_outs is not None: 
                ...
                deltaF = self.modify_feature_gates(x, aligned_feats)
                x = x + deltaF
                gouts['gates'] = torch.ones((1,1,1,1))
