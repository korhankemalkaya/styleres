...
class ResLayers(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, actv = True):
        super().__init__()
        ...
        if actv:
            actv_layer = nn.LeakyReLU(0.2)
        else:
            actv_layer = nn.Identity()
        self.res_layer = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, (3, 3), (1, 1), 1, bias=True), nn.LeakyReLU(0.2),
            nn.Conv2d(out_channels, out_channels, (3, 3), stride, 1, bias=True) , actv_layer)
...
class FeatureAlignment(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        t_channel = out_channels
        self.first_layer = nn.Conv2d(in_channels, t_channel, kernel_size=1, padding=0, bias=True)

        self.conv1 =  nn.Sequential(*[ResLayers(t_channel,t_channel,1), ResLayers(t_channel,t_channel,1), ResLayers(t_channel,t_channel,1)])
...
class GateNetwork(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.convs = nn.Sequential(*[ResLayers(in_channels,in_channels,1), ResLayers(in_channels,out_channels,1), ResLayers(out_channels,out_channels,1,False) ])
        self.convs2 = nn.Sequential(*[ResLayers(in_channels,in_channels,1), ResLayers(in_channels,out_channels,1), ResLayers(out_channels,1,1, False) ])
...
g_e_concat_shape={128:320, 64: 640, 32:768}
e_shape = {128:64, 64: 128, 32:256}
@persistence.persistent_class
    ...
    def __init__(self,
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if resolution == embed_res:
            ...
            out_channels = g_e_concat_shape.get(embed_res) - e_shape.get(embed_res)
            self.modify_feature_alignment = FeatureAlignment(in_channels=in_c, out_channels=out_channels)
            #self.modify_feature_extraction = FeatureExtraction(in_channels=512, out_channels=512)
            self.modify_feature_gates = GateNetwork(in_channels=out_channels, out_channels=out_channels)
