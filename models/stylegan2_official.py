    ...
    def __init__(self,
        ...
        **layer_kwargs,                         # Arguments for SynthesisLayer.
        ...
        if resolution == embed_res:
            ...
            self.modify_feature_extraction = FeatureExtraction(in_channels=640, out_channels=512)
    ...
    def forward(self, x, img, ws, conditions=None, noise=None, weight_deltas = None, highres_outs=None, return_f = False,
        ...
        if self.in_channels == 0:
            ...
            if x.shape[-1] == self.embed_res and return_f:
                    return x, None, None
            #HighResFeat Generator Modification (burası da atlanabilir)
            if x.shape[-1] == self.embed_res and highres_outs is not None: 
                ...
                aligned_feats = torch.cat((highres_outs[f'{self.embed_res}x{self.embed_res}'],highres_outs['inversion']), dim=1)
                aligned_feats = self.modify_feature_extraction(aligned_feats)
                #pytorch_total_params = sum(p.numel() for p in self.modify_feature_extraction.parameters())
                #print("PARAM COUNT : " + str(pytorch_total_params))
