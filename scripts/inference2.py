...
def get_all_latents(net, data_loader, args, is_cars=False):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            save_image(imgs[0], x[0], args.save_dir , ori_imgs[i])
...
def save_image(img, orig, save_dir, name):
    #result = tensor2im(img).resize((256,256))
    im1 = tensor2im(orig).resize((256,256))
    im2 = tensor2im(img).resize((256,256))
    dst = Image.new('RGB', (im1.width + im2.width + 2, im1.height))

    dst.paste(im1, (0, 0))
    dst.paste(im2, (im1.width + 2, 0))
    im_save_path = os.path.join(save_dir, name)
    Image.fromarray(np.array(dst)).save(im_save_path)
