...
def get_all_latents(net, data_loader, args, is_cars=False):
    ...
    ori_imgs.sort()
    interfacegan_edits = ['age', 'smile', 'pose']
    if args.edit in interfacegan_edits:
        direction = f'editings/interfacegan_directions/{args.edit}.pt'
    ...
    directions = {
        'eye_openness':            (54,  7,  8,  25),
        'ganspace_smile':                   (46,  4,  5, -20),
        'beard':                   (58,  7,  9,  -20),
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            # if args.edit == 'interfacegan_smile':
            #     latents = latents + factor * interfacegan_direction
            #latents = latents + factor * interfacegan_direction
            #latents = editor.apply_ganspace_(latents, ganspace_pca, [directions["white_hair"]])
            if args.edit in ['smile', 'age', 'pose']:
                latents = latents + factor * interfacegan_direction
            elif args.edit in ['eye_openness', 'ganspace_smile', 'beard', 'lipstick']:
                latents = editor.apply_ganspace_(latents, ganspace_pca, [directions[args.edit]])
            #latents = editor.apply_sefa_(latents, indices=[6,7,8], start_distance=15.0, end_distance=15.0, step=1)
            #latents = latents + factor * interfacegan_direction
            if (args.save):
...
def save_image(img, save_dir, name):
    ...
    result = tensor2im( F.interpolate(img.unsqueeze(0), (256,256)).squeeze(0))
