...
from utils.misc import DictAction, parse_config, update_config, assert_config, infer_configs
...
def parse_args():
    ...
    parser.add_argument('--continue_train', action="store_true", default=False,
                        help='Continue training with the model on the working directory')
...
def main():
    ...
    if config.continue_train:
        ...
        if (workdir[-1] != '/'):
            workdir = workdir + '/'
        filepath = f"{workdir}latest.pth"
        runner.load(filepath=filepath,
                    running_metadata=True,
                    learning_rate=True,
                    optimizer=True,
                    running_stats=False)
