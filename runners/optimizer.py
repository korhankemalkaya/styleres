...
def build_optimizers(opt_config, runner):
    ...
    optimE_models= [runner.models["encoder"]]
    ALLOWED_E_MODELS = ["mapping", "mixer"]
    for name, config in opt_config.items():
        if name in ALLOWED_E_MODELS: #Mixer and Mapping Networks are Added
            assert config is None, "Config should be None for the rest of sub E modules"
            optimE_models.append(runner.models[name])

    for name, config in opt_config.items():
        if not name or not config or name == 'encoder':
            continue
        ...
        if name == 'encoder':
            runner.optimizers[name] = build_optimizer(config, optimE_models)
        else:
