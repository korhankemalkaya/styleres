...
from .controllers import summary_writer
...
class BaseRunner(object):
    ...
    def __init__(self, config, logger):
        ...
        self.smile_n_loader = None
        self.smile_p_loader = None
    ...
    def _create_meanw(self, path, n_samples=1e5):
        """
        Calculates the meanw from n samples and saves them to path.
        For now only main GPU is used to create mean w.
        """
        ...
        with torch.no_grad():
            z =  torch.randn((n_samples, 512), device=torch.cuda.current_device())
            w = G.mapping(z, c=None, repeat_w = False)
            meanw = torch.mean(w, dim=0)
        meanw = meanw.detach().cpu().numpy()
        np.save(path, meanw)

    def create_meanw(self):
        """
        Meanw is created as a numpy array.
        """
        meanw_lw = self.config.loss['e_loss_kwargs']['meanw_lw']
        #recalculate_meanw = self.config.meanw_settings['recalculate_meanw']
        path = self.config.meanw_settings['meanw_path']
        n_samples = self.config.meanw_settings['n_samples']
        if meanw_lw == 0:
            self.meanw = None
            self.logger.info("Mean w vector is NOT created.")
            return False
        # elif (recalculate_meanw):
        #     self._create_meanw(path, n_samples=n_samples)
        #     dist.barrier()  #Wait the meanw calculation
        #     self.meanw = np.load(path)
        #     self.logger.info("Mean w vector is successfully RECALCULATED.")
        #     return True
        elif meanw_lw > 0 and not (os.path.exists(path)):
            self._create_meanw(path, n_samples=n_samples)
            dist.barrier()  #Other GPUs, wait the meanw calculation
            self.meanw = np.load(path)
            self.logger.info("Mean w vector is successfully CREATED.")
            return True
        elif meanw_lw > 0 and os.path.exists(path):
            self.meanw = np.load(path)
            self.logger.info("Mean w vector is successfully LOADED")
            return True

    def create_perceptual(self):
        percept_loss = self.config.loss['e_loss_kwargs']['perceptual_lw'] > 0
        test_optim = self.config.test_time_optims != 0
        self.perceptual_model = None
        if percept_loss or test_optim:
            perceptual_kwargs = self.config.loss['perceptual_kwargs']
    ...
    def build_models(self):
        ...
        lr_config = dict()
        opt_config = dict()
        if (self.config['use_latent_disc'] == False):
            del self.config.modules['latent_disc']
        if (self.config['use_disc2'] == False):
            del self.config.modules['discriminator2']
        if (self.config['mapping_method'] == 'pretrained'):
            del self.config.modules['mapping']
        if (self.config.create_mixing_network is False):
            del self.config.modules['mixer']
        ...
        for modelname,model in self.models.items():
            ...
            self.logger.info(modelname + ' model size: {:.3f}MB'.format(size_all_mb))
            params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            self.logger.info(f'{modelname} params: {params}')
    ...
    def build_controllers(self):
        ...
        summary_writer.setup(self.work_dir, self.rank)
    ...
    def train(self, **train_kwargs):
        ...
        self.build_loss()
        self.use_meanw = self.create_meanw()
        if self.use_meanw:
            self.meanw = torch.from_numpy(self.meanw).to(device=torch.cuda.current_device())
        self.create_perceptual()
        self.logger.print()
        ...
        #change_last_two = self.config['loss']['e_loss_kwargs']['ratio_lw'] > 0 
        wrand_edit_factor = self.config.wrand_edit_factor
        interface_prob = self.config.interface_prob
        randw_prob = self.config.randw_prob
        edit_prob = self.config.edit_prob
        bs = self.config.batch_size
        device = torch.cuda.current_device()
        if interface_prob > 0:

            interfacegan_edit_directions = {}
            interfacegan_edit_directions = torch.zeros((3,512), device=device)
            for idx, edit in enumerate(['age', 'smile', 'pose']):
                if edit is 'pose':
                    interfacegan_edit_directions[idx] =  torch.load(f'editings/interfacegan_directions/{edit}.pt')[0][0].cuda()
                else:
                    interfacegan_edit_directions[idx] = torch.load(f'editings/interfacegan_directions/{edit}.pt').cuda()
        while self.iter < self.total_iters:
            ...
            for key in data_batch:
                if key != 'name':
                    ...
                    # if key == 'valid': #For checking if the masks are correct
                    #     valid_numpy = data_batch['valid'].numpy()
                    #     for i in range(valid_numpy.shape[0]):
                    #         cv2.imwrite(f'{self.rank}_{self.iter}_{i}.png', np.squeeze(valid_numpy[i], axis=0)*255)
                    data_batch[key] = data_batch[key].cuda(
                        torch.cuda.current_device(), non_blocking=True)
            
            #Random values
            factor = torch.zeros((bs,1), device=device)
            direction = torch.zeros((bs, 512), device=device)
            edit_name = [''] * bs
            for b in range(bs):
                isedit = np.random.rand(1)
                if isedit > edit_prob:
                    factor[b] = 0
                    direction[b] = 0
                    edit_name[b] = None
                else:
                    israndw = np.random.rand(1)
                    if israndw < randw_prob:
                        factor[b] = ( torch.rand( (1,), device=device) + wrand_edit_factor) # +2 
                        z = torch.randn((1,self.config.latent_dim), device=torch.cuda.current_device())
                        direction[b] = self.runM(z)
                        edit_name[b] = 'randw'
                    else:
                        factor[b] = ( torch.rand( (1,), device=device) * 10 - 5)
                        interface_selection = np.random.randint(0,3)
                        direction[b] = interfacegan_edit_directions[interface_selection]
                        edit_name[b] = 'interface'

            
            data_batch['factor'] = factor
            data_batch['direction'] = direction
            data_batch['edit_name'] = edit_name
            self.train_step(data_batch, **train_kwargs)
            self.seen_img += self.batch_size * self.world_size
            self.timer.post_execute(self)
            #Example custom save functionality
            #if (self.iter < 5):
            #    self.save_now = True
    ...
    def save(self,
        ...
        for name, model in self.models.items():
            if name not in ignore_models:
                checkpoint['models'][name] = self.get_module(model).state_dict()
        # Running metadata.
        if running_metadata:
            checkpointer_idx = -1
            for i in range(len(self.controllers)):
                if self.controllers[i].name == 'Checkpointer':
                    checkpointer_idx = i
                    break
            checkpoint['running_metadata'] = {
                'iter': self.iter,
                'seen_img': self.seen_img,
                'best_fid' : self.controllers[checkpointer_idx].best_fid
            }
        # Optimizers.
        if optimizer:
    ...
    def load(self,
             ...
             noise = False,
             map_location='cpu',
             load_model=['encoder', 'generator', 'generator_smooth', 'discriminator']):
        ...
        for model_name, model in self.models.items():
            ...
            if model_name not in checkpoint['models']:
                if (model_name == 'discriminator2'):
                    self.logger.warning(f'Model `{model_name}` is not included in '
                                        f'the checkpoint, and hence weights will be same as discriminator.')
                    load_name = 'discriminator'
                else:
                    self.logger.warning(f'Model `{model_name}` is not included in '
                                        f'the checkpoint, and hence will NOT be '
                                        f'loaded!')
                    continue
            if load_name not in load_model:
                continue
            state_dict = _strip_state_dict_prefix(checkpoint['models'][load_name])
            missing_keys = model.load_state_dict(state_dict, strict=False)
            self.logger.info(f'Model {model_name} has the following missing keys: {missing_keys[0]}')
            self.logger.info(f'  Successfully loaded model `{model_name}`.')
        if noise:
            self.noise_list = []
            for key, value in checkpoint['models']['generator_smooth'].items():
                if 'noise_const' in key:
                    self.noise_list.append(value)
            self.logger.info(f'  Successfully loaded model Generator Noise.')
        ...
        if running_metadata:
            ...
            if 'running_metadata' not in checkpoint:
                ...
                best_fid = checkpoint['running_metadata']['best_fid']
                self.controllers[checkpointer_idx].best_fid = best_fid
        # Load optimizers.
        if optimizer:
