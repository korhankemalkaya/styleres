...
from editing.styleclip.model_ours import Generator
...
def parse_args():
    ...
    parser.add_argument("--weight_deltas_path", type=str, default="./weight_deltas",
                        help="Root path holding all deltaF (obtained by running inference.py).")
    ...
    parser.add_argument("--num_alphas", type=int, default=3)
    ...
    parser.add_argument("--network", type=str, default="e4e", help="e4e, hfgi, ours")
...
def load_stylegan_generator(args):
    ...
    if args.network == 'ours':
        ckpt_modifications = torch.load('/media/hdd2/adundar/hamza/hyperstyle/pretrained_models/best.pth')
        for n,p in ckpt_modifications['models']['generator_smooth'].items():
            if 'modify_feature_gates' in n:
                checkpoint['g_ema'][n[14:]] = p
...
def edit_image(image_name, latent, stylegan_model, global_direction_calculator, args):
    ...
    efeats = np.load(os.path.join(args.weight_deltas_path, image_name.split(".")[0] + ".npy"), allow_pickle=True)
    efeats = torch.from_numpy(efeats).cuda()
